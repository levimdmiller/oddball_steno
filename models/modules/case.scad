              ...
              difference(){
                    ...
                    linear_extrude(plateUndersideClearance){
                      ...
                      for(keyPosition = keyPositions){
                        if(keyPosition.z == "pmw")
                          translate(
                            [keyPosition.x + 0.5, (keyPosition.y + 0.5) * keyHeight] * 1U + 
                            [PLATE_BEZEL, PLATE_BEZEL]
                          ) {
                            offset(delta = CASE_PLATE_TOLERENCE)
                ...
                difference(){
                  union(){
                    for(boltPosition = boltPositions)
                      translate(
                        [boltPosition.x, boltPosition.y * keyHeight, 0] * 1U + 
                        [PLATE_BEZEL, PLATE_BEZEL, caseBottomThickness]
                      ){
                        
                        difference(){
                          union() {
                            housingRadius = CASE_BOLT_HOUSING_WIDTH 
                              + BOLT_DIAMETER * 0.5 
                              + BOLT_TOLERENCE;
                            ...
                            if(flangeStandoffs){
                              ...
                              cylinder(
                                h = plateUndersideClearance - PCB_PLATE_OFFSET_Z - 2, 
                                r1 = housingRadius * 1.5, 
                                r2 = housingRadius
                              );
                            }
                            if (flangeThroughPcb) {
                              cylinder(
                                h = plateUndersideClearance, 
                                r = CASE_BOLT_HOUSING_WIDTH + BOLT_DIAMETER * 0.5 + BOLT_TOLERENCE
                              );
                            }
                          }
                          cylinder(
                            h = plateUndersideClearance, 
                            r = BOLT_DIAMETER * 0.5 + BOLT_TOLERENCE
                          );
                        }
                      }
                  }
